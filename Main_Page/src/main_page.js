...
async function loadGithubData() {
    try {
        ...
        if (repositoryList)
            repositoryList.innerHTML = repositories.length ? repositories.slice(0, 6).map(repositoryCard).join("") : "<p class=\"loading-message\">No public repo yet</p>";
    }
    catch (error) {
        if (repositoryList)
            repositoryList.innerHTML = "<p class=\"loading-message\">GitHub not avaiable.</p>";
        console.error(error);
    }
}
loadGithubData();
