...
async function loadGithubData(): Promise<void> {
	try {
		...
		if (repositoryList) repositoryList.innerHTML = repositories.length ? repositories.slice(0, 6).map(repositoryCard).join("") : "<p class=\"loading-message\">Публичных репозиториев пока нет.</p>";
	} catch (error) {
		if (repositoryList) repositoryList.innerHTML = "<p class=\"loading-message\">GitHub временно недоступен. Загляните прямо в профиль curettu.</p>";
		console.error(error);
	}
}
